...
def main() -> None:
    ...
    # Check if we should use RL training
    is_rl_gymnasium = args.agent == 'rltraining'
    
    if is_rl_gymnasium:
        # Use clean Gymnasium-based training with proper swarm management
        algorithm_type = "PPO"  # Standard PPO only
        logger.info(f"Using Gymnasium-based RL training with {algorithm_type} and swarm orchestration...")
        
        from agents.rl.rl_training import RLTrainingSwarmGymnasium
        swarm = RLTrainingSwarmGymnasium(
            agent=args.agent,
            ROOT_URL=ROOT_URL,
            games=games,
            tags=tags,
        ...
        # Use standard swarm for non-RL agents
        swarm = Swarm(
            args.agent,
            ROOT_URL,
            games,
